...
class Dapur_menu extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if($transaksi){
            ...
            $cekdatak = $this->db->get_where("dapur_pengeluaran", ["tanggal" => $tanggal, "user_id" => $user["id"]])->num_rows();
            if($cekdatak == 0){
                $trx = 1;
            } else {
                $query = $this->db->select("transaksi")->from("dapur_pengeluaran")->where("tanggal", $tanggal)->where("user_id", $user["id"])->order_by("id_dp", "desc")->limit(1)->get();
        ...
        if ($tanggal AND $trx) {
            $ceklaporan = $this->db->get_where("dapur_pengeluaran", ["tanggal" => $tanggal, "user_id" => $user["id"], "transaksi" => $trx])->num_rows();
            if ($ceklaporan > 0) {
                $data["olah"] = "edit";
                $data["biaya_rutin"] = $this->db->get_where("dapur_pengeluaran", ["pengeluaran" => 1, "tanggal" => $tanggal, "user_id" => $user["id"], "transaksi" => $trx])->result_array();
                $data["biaya_lain"] = $this->db->get_where("dapur_pengeluaran", ["pengeluaran" => 2, "tanggal" => $tanggal, "user_id" => $user["id"], "transaksi" => $trx])->result_array();
                $data["pembelian"] = $this->db->get_where("dapur_pembelian", ["tanggal" => $tanggal, "user_id" => $user["id"], "transaksi" => $trx])->result_array();
                $data["total_br"] = $this->db->select_sum('nilai')->where(["tanggal" => $tanggal, "user_id" => $user["id"], "pengeluaran" => 1, "transaksi" => $trx])->get('dapur_pengeluaran')->row()->nilai;
                $cek_bl = $this->db->get_where("dapur_pengeluaran", ["pengeluaran" => 2, "tanggal" => $tanggal, "user_id" => $user["id"], "transaksi" => $trx])->num_rows();
                if($cek_bl == 0){
                    $data["total_bl"] = 0;
                } else {
                    $data["total_bl"] = $this->db->select_sum('nilai')->where(["tanggal" => $tanggal, "user_id" => $user["id"], "pengeluaran" => 2, "transaksi" => $trx])->get('dapur_pengeluaran')->row()->nilai;
                }
                $cek_pb = $this->db->get_where("dapur_pembelian", ["tanggal" => $tanggal, "user_id" => $user["id"], "transaksi" => $trx])->num_rows();
                if($cek_pb == 0){
                    $data["total_pb"] = 0;
                } else {
                    $data["total_pb"] = $this->db->select_sum('nilai')->where(["tanggal" => $tanggal, "user_id" => $user["id"], "transaksi" => $trx])->get('dapur_pembelian')->row()->nilai;
                }
                ...
                $data["no_dok"] = generate_no_dok($tanggal, $user["id"], $trx);
            } else {
        ...
        layout2("dapur/input_barang", $data);
    }


    public function tambah($tanggal, $trx)
    {
        $nickname =  $this->session->userdata('nickname');
        $user = $this->db->get_where("master_user", ["username" => $nickname])->row_array();
        $id_user = $user["id"];
        ...
        redirect('dapur/dapur_menu?tanggal=' . $tanggal."&trx=".$trx);
    }

    public function edit($tanggal, $trx)
    {
        $nickname =  $this->session->userdata('nickname');
        $user = $this->db->get_where("master_user", ["username" => $nickname])->row_array();
        $id_user = $user["id"];
    ...
    public function filter()
    {
        ...
        layout2("dapur/input_dapur", $data);
    }
    ...
    public function tambah_dp($tanggal, $id, $trx)
    {
        $nickname =  $this->session->userdata('nickname');
        ...
        redirect('dapur/dapur_menu/rolling_dapur?tanggal='. $tanggal."&id=".$id."&trx=".$trx);
    }


    public function edit_dp($tanggal, $id, $trx)
    {
        $nickname =  $this->session->userdata('nickname');
        $user = $this->db->get_where("master_user", ["id" => $id])->row_array();
        $rol = $this->db->get_where("master_user", ["username" => $nickname])->row_array();
        ...
        $getRoll = $this->db->select('tanggal, rolling, no_dok, user_id, SUM(nilai) as total_nilai')->from('dapur_pengeluaran')->where([ "tanggal" => $tanggal, "user_id" => $id,"pengeluaran" => 1, "transaksi" => $trx])->order_by('tanggal')->group_by("user_id")->group_by("no_dok")->group_by('tanggal')->group_by('rolling')->get()->row_array();
        if ($this->input->post('biaya_bl1')) {
            $biaya_bl1 = $this->input->post("biaya_bl1");
            $bl1 = [];
            foreach ($biaya_bl1 as $key => $value) {
