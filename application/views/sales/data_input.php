            ...
            <div class="card">
                <div class="card-header">
                    <div class="card-title">
                        <div class="mb-0 text-center"><?php echo $title; ?> Periode <?php echo TglIndo($year.'-'.$month.'-'); ?></div>
                    </div>
                </div>
                <div class="card-body">
                    <form action="<?php echo base_url('sales/validasi_data/caridata'); ?>" method="post">
                        <div class="row">
                            <div class="col-lg-6">
                ...
                <div class="card-header">
                    <div class="card-title">
                        <div class="d-flex justify-content-between">
                            <div class="mb-0"><?php echo $title; ?></div>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                                    ...
                                    <?php if ($row['outlet_id']) { ?>
                                        <tr>
                                            ...
                                            <?php for ($i = 1; $i <= $tanggal; ++$i) { ?>
                                                ...
                                                <td>
                                                    <?php if (!$cekdata) { ?>
                                                        <div class="badge bg-secondary pt-1"><i class="bx bx-radio-circle"></i></div>
                                                    <?php } else { ?>
                                                        <?php
                                                            $cekdata2 = $this->db->get_where('sales_laporan', ['tanggal' => $tanggal2, 'user_id' => $row['id'], 'status' => 1])->num_rows();
                                                        if (!$cekdata2) {
                                                            ?>
                                                            <div class="badge bg-danger pt-1"><i class="bx bx-x"></i></div>
                                                        <?php } else { ?>
                                                            <div class="badge bg-success pt-1"><i class="bx bx-check"></i></div>
                                                        <?php }?>
                                                    <?php }?>
                                                </td>
                                            <?php } ?>
